var rule = {
    ...
    double:false,
    推荐:'*',
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&.live_list_content li');tabs.forEach(function(it){var pz=pdfh(it,'td:eq(0)&&Text');var ps=pdfh(it,'.leixing&&Text');var pk=pdfh(it,'td:eq(2)&&Text');var img=pd(it,'img&&src');var timer=pdfh(it,'.s1_time&&Text');var url=pd(it,'a&&href');items.push({desc:timer+'  '+ps,title:pz+'🆚'+pk,pic_url:img,url:url})});setResult(items);",
    二级:{
